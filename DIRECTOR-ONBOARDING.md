...
## Addendum, 31 August 2026 — added on migration, nothing above is edited

*The 278 lines above are the file as its author wrote it, byte-identical to
`eql-source` `claude/eq-map-export-proposal-oe8m6l` at `0d094560`. Nothing is
struck and nothing is rewritten. This section exists because two statements
above are now false in a way a reader could act on, and the author's own rule is
that a superseded line left readable is a hazard rather than a record.*

**1. §2's topology no longer holds for a local Director.**

> *"The Director cannot initiate. `SendMessage` is refused for this cloud
> session. Your branch is your entire voice."*

That was true, and it is still true of a **cloud** Director. **The Director post
moved to a local session on 31 August 2026, and a local Director can message
sessions directly.** Read a fresh listing before every send and skip every row
marked offline — that half of §2 stands unchanged and matters more now, not less.

**What did not change:** local sessions still die when the PC goes off, that
still includes Relay 0, and when the PC is off **the owner is still the relay**.
Keep `## PASTE TO CLOUD SESSIONS` current at the end of the record.

**2. §7's pointer is one hop short.**

> *"The Director's record is `HANDOFF.md` on `claude/eq-map-export-proposal-oe8m6l`."*

**The record moved to `samusmylove47-maker/Director`, `main`, on 31 August 2026.**
That branch is still alive and still readable; its copy of `HANDOFF.md` is frozen
and carries a stub pointing here. The commands in §7 still work — they now reach
a redirect rather than the live file.

**3. §7's sibling clones are not on this machine.** `dpeek`, `epeek`, `bpeek`,
`cpeek` and `spk` were in the *cloud* session's scratchpad. The local Director
keeps its own read-only clones under `peers/`, which is gitignored. Fetch from
them; never push to them.

**Nothing else in this file is known to be stale.** §0 and §6 in particular are
unaffected by the move and are the reason it was copied across rather than
rewritten.
